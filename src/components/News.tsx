          ...
          {news.map((n, i) => (
            <Reveal key={n.title} delay={i * 0.1}>
              <Card3D className="h-full">
                <article className="bg-surface/30 p-8 h-full border border-white/5 group cursor-pointer hover:bg-surface/50 transition-colors duration-500 rounded-xl overflow-hidden">
                  <div className="flex items-center gap-3 text-xs text-muted-foreground uppercase tracking-wider mb-6">
                    <span>{n.date}</span>
                    <span className="w-1 h-1 rounded-full bg-red-600" />
                    <span className="text-red-600 font-bold">{n.cat}</span>
                  </div>
                  <h3 className="font-display text-2xl leading-tight mb-4 group-hover:text-red-600 transition-colors duration-500">
                    {n.title}
                  </h3>
                  <p className="text-sm text-muted-foreground leading-relaxed mb-6">{n.excerpt}</p>
                  <div className="flex items-center gap-3 text-sm">
                    <span className="w-8 group-hover:w-16 h-px bg-red-600 transition-all duration-700" />
                    <span className="text-red-600 font-bold">Read</span>
                  </div>
                </article>
              </Card3D>
            </Reveal>
